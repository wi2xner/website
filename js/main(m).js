...
async function getRewardsEx1() {
    ...
    if (
        ...
        const xx1 = await myContract2.methods.getAccountDividendsInfo(userWalletAddress).call({ from: userWalletAddress });
        const xxInEth1 = (xx1[3]) / (1000000000000000000);
...
async function getInventory() {
    ...
    const nftObj = await Promise.all(NFTJSONs.map(a => fetch(a).then(res => res.json())))
